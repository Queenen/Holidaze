...
import styles from "./RandomPicks.module.css";
import { Link } from "react-router-dom";

const RandomPicks = ({ showEditButton = true }) => {
  const [randomVenues, setRandomVenues] = useState([]);

  useEffect(() => {
    const fetchRandomVenuesData = async () => {
      try {
        const venues = await fetchRandomVenues();
        if (venues && Array.isArray(venues)) {
          setRandomVenues(venues);
        } else {
